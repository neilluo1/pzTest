...
def iozone_reports():
    ...
    reports = []
    for template in os.listdir(report_path):
        if 'iozone-report' in template and os.path.isfile(os.path.join(report_path, template)):
            reports.append(template)

    return render_template('iozone.html', reports=reports)


@app.route('/iozone/<report_html>')
def iozone_report(report_html):
    return render_template(os.path.join(report_html))
